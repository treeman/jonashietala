...
One might wonder, where do the symbols appear? Are they mostly next to characters, or next to other symbols? Here's a table of where the symbols end up, and what symbol bigrams they appear (ignoring double symbols like `||`), using the Code source:
 ...
 Char       Count           Next to alpha          Next to symbol         Common bigrams
------    --------         ---------------        ----------------       -------------------------------
...
![Alpha frequencies from the code and keylog corpus, and a large English and Swedish dataset](/images/t-34/combined-se-en-letter-freq.svg)

Optimizing alphas is an area where layouts have focused a lot of attention on. Some notes from data:
...
3. My Vim usage makes some unusual keys quite common. Apparently I'm a `j`/`k` spammer (up/down)---which isn't a good habit to have as there are more efficient ways to move vertically. `w` and `b` (forward and backwards a word) and `:w` (save) are used a lot. I also seem to mistype a lot as `u` is very common (undo).
