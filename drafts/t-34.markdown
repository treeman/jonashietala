...
- I use `/` as a regex search in Vim a lot.
- `=` is on base as it makes ` = ` very comfortable, which is the primary usage for it.

You could make a case for some other choices here; I'm considering swapping `/` and `"`, and I've gone back and forth on `=` on base several times. Still, this feels quite good---especially `(`, `)` and `_`.
...
One change I'm considering is moving `&` to a better spot, as it's just so common in Rust.
